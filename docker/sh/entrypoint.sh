...
xdebug.start_upon_error=${XDEBUG_START_UPON_EfRROR:-no}
EOF'
...
if [ -f "$PROJECT_ROOT/composer.json" ]; then
  cPrint status "A composer.json file was found."
  ...
  if [ ! -d "$PROJECT_ROOT/vendor" ]; then
      ...
      cPrint status "Composer dependencies seem to be installed."
  fi
else
    cPrint info "A composer.json was not found. Skipping..."
fi
